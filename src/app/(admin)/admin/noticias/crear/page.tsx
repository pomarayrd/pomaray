...
function NewPostsPage() {
	...
	const { isSuccessOpen, openSuccessModal, openSuccessModalChange } =
		useSuccessModal();
	...
	if (!user) {
		return (
			<div className="flex flex-center min-h-[80vh]">
		...
		<>
			<section className="py-12">
				...
				<div>
					...
					{response?.errors?.content && (
						<Message className="min-w-full text-xs" color="danger">
							{response?.errors?.content}
						</Message>
					)}
				...
				<div className="py-4">
					<ActionModal
						intensity="hard"
						title="Publicar una nueva noticia"
						onConfirm={handleConfirmButton}
					>
						<Text>
							Estas seguro que desea publicar esta noticia a todo el publico.
						</Text>
						<Text as="small" size="label-xs">
							Puede ser editada después de su publicación.
						</Text>
					</ActionModal>
				</div>
			...
			<SuccessModal
				...
				children={
					<>
						<Text>
							Se ha publicado la noticia correctamente. Disfrute del confetti!
						</Text>
